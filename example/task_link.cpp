...
void test_task_link()
{
	...
	task<task_node<int>, task_node<std::string, int>> t2 = t1.then(fn_second);
	task<task_node<int>, task_node<std::tuple<int, std::string>, std::string>> t3 = t2.then(imm_context, fn_third);
	task<task_node<int>, task_node<void, int, std::string>> t4 = t3.then(imm_context, fn_four);
